...
Avant tout, et puisque ce projet est basé sur NetHack, nous allons donner une
brève description de ce jeu. Le joueur choisit en début de partie le rôle, la
race et le sexe de son personnage, son dieu ou sa déesse lui donne alors sa
quête : rechercher l'Amulette qu'il désire détenir en sa possession.
Le joueur doit alors explorer des donjons (le jeu est en effet composé de
plusieurs donjons), peuplés de monstres et de trésors, pour accomplir sa
quête. Chaque donjon contient plusieurs niveaux et le joueur avance dans
l'exploration en descendant dans les niveaux. Chaque niveau se présente sous
la forme d'une grille composées de salles et de couloirs les reliant et le jeu
se déroule au tour par tour.

Au cours de sa quête le joueur va interagir avec des monstres contre lesquels
il faudra se défendre, des pièges qu'il essaiera d'éviter, des objets qu'il
pourra utiliser et la faim qu'il devra combattre (à l'aide de pommes).

\paragraph{}
À chaque lancement d'une nouvelle partie, les donjons sont construits de
manière aléatoire dans un mélange de niveaux aléatoires et de niveaux spéciaux
préconstruits. Les niveaux croissent en difficulté à mesure que le joueur
descend les escaliers pour passer aux niveaux suivants et de nombreux événements
inattendus ponctuent le déroulement de la partie pour surprendre le joueur en
quête de survie.
...
écrire en {\em bash} (scripts de générations de graphiques ou de
lancement d'un grand nombre de parties) ou en {\em Perl}
(le viewer et l'outil toTikZ), à cause de leur nature très variable
...
meilleure maintenabilité. Nous n'avons pas fourni cet effort car, en
discutant avec M. Le Borgne, nous avons compris qu'il préférait obtenir
plus de graphiques fournissant des informations plutôt que d'avoir une
génération de graphe en {\em C} mais moins complète.
...
on souhaite brancher un bot (cf. page de manuel pty(7)).
...
superpositions de menus lorsque nécessaire (voir libtermcap). Cela
...
(fenêtre de dialogue propres à chacune d'elles par exemple). À moins de
...
Un exemple détaillé de création d'un patch est disponible dans le manuel
...
'lecture seule' (ce programme a plusieurs modes d'exécution) et interprétera
le flux pour afficher la carte correspondant à la partie en cours.
...
\begin{figure}[H]
  \center{\includegraphics[width=\textwidth]{graph_level_reached.png}}
...
\begin{figure}[H]
  \center{\includegraphics[width=\textwidth]{impulse_level_reached.png}}
...
avantages ont en revanche un coût en complexité très élevé (il devrait
...
qui présentait une spécificité (pour certains niveaux, il était
...
dans la base de données à l'issue des parties jouées. Regrouper l'ensemble des
