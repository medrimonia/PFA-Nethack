...
le script d'installation \verb!nh-setup.sh!. Il est possible de modifier la
recherche automatique des patchs en modifiant les lignes suivantes du script
\verb!nh-setup.sh! (à partir de la ligne 67):
\begin{verbatim}
...
sert à désactiver les monstres, un autre désactive les pièges, etc. Ce
découpage en petits patchs permet de pouvoir les réutiliser d'un mode de jeu à
un autre, ainsi notre mode de jeu de recherche des portes secrète décrit
précédemment utilise un sous-ensemble de ces patchs. Un autre mode de
recherche de porte secrète spécifique à la recherche dans une salle utilise ce
...
Un patch est en fait le résultat d'un \verb!diff!\footnote{man diff(1)} entre
deux version d'un fichier. Ce patch peut alors être appliqué avec la commande
\verb!patch!\footnote{man patch(1)} à l'ancien fichier afin de donner le
nouveau.
Il existe plusieurs façon de créer un tel fichier, l'une d'entre elle utilise
...
personnalisée et plutôt libre, basées sur des rencontres régulières avec notre
...
En effet, le module de visualisation des parties jouées par exemple a été écrit
en perl car il aurait demandé un temps de développement plus conséquent pour
être écrite en C (il aurait fallu gérer des contraintes bas niveau qui
...
production de graphes. Il est donc parfois nécessaire de lire les README
...
été fait car après concertation avec notre client, Monsieur Le Borgne, il a
