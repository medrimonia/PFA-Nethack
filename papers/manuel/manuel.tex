...
Tout d'abord, le jeu doit être lancé (avec les paramètres nécessaires, voir ci-après) avant le bot. NetHack attendra alors qu'un bot se connecte. Le bot choisi ou le dummy-client peuvent alors être lancés manuellement.

\subsection{Lancement d'une série de parties automatiquement}
...
\begin{itemize}
	\item \verb!NH_MM_SOCKPATH! : spécifier un chemin pour la socket unix à créer. Par défaut, le middleman crée /tmp/mmsock.
	\item \verb!NH_MM_REPLAY! : si mise à une valeur différente de 0, cette variable active l'enregistrement de la partie dans le fichier nethackdir/replay. Désactivée par défaut.
	\item \verb!NH_MM_LOGGING! : si mise à une valeur différente de 0, cette variable active l'enregistrement de logs du middleman dans le fichier nethackdir/mm.log. Désactivée par défaut.
...
Un fichier nethackdir/replay peut être générer en mettant la variable d'environnement \verb!NH_MM_REPLAY! à une valeur différente de 0 lors du lancement du jeu. Le programme viewer/viewer.pl permet d'exploiter ce fichier afin de visualiser la partie enregistrée. Voir le README correspondant.
...
L'interface ne supporte pas les commandes commençant par le caractère '\verb!#!'. Ceci est dû au code original de NetHack qui cherche à réaliser des traîtements particuliers pour ce type de commande (auto completion, etc) que nous n'avons pas jugé utile d'implémenter. Tout caractère '\verb!#!' est donc ignoré par l'interface.

Un bot peut envoyer plusieurs commandes à la foi ce qui peut se montrer utile pour diminuer le nombre d'échanges avec le jeu. Par exemple, envoyer les commandes '\verb!jjkl!' aura le même effet que d'envoyer chacune des lettres séparément et fera bouger le joueur deux fois vers le bas, une fois vers le haut et une fois vers la droite.
