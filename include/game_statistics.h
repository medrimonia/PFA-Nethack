...
/** @brief Initialize the game statistic module */
void gs_init();

/** @brief Free all the resources dedicated to the game statistic module */
void gs_terminate();
...
/**
 * @brief Add a door to the number of secret doors that can be found. 
 *
 * This function must be called for the doors which haven't been added in an
 * update_nb_sdoor().
 */
void statistic_add_sdoor();

/** @brief Shall be called once by secret door discovery. */
void statistic_add_sdoor_discovery();

/** 
 * @brief Add a door to the number of secret corridors that can be found. 
 *
 * This function must be called for the secret corridors which haven't been
 * added in an update_nb_sdoor().
 */
void statistic_add_scorr();

/** @brief Shall be called once by secret corridor discovery */
...
/** 
 * @brief Must be called at least once each time a new square is reached
 * (might be called more than once on the same square).
 */
void update_reached_squares();

/** @brief Shall be called when a seed is used to set game random */
void gs_set_seed(int);

/** 
 * @brief Return the number of secret doors reachable in all the level
 * reached. 
 */
int get_nb_sdoors();

/** @brief Return the number of secret doors found during the game. */
int get_nb_sdoors_found();

/**
 * @brief Return the number of secret corridors reachable in all the level
 * reached.
 */
int get_nb_scorrs();

/** @brief Return the number of secret corridors found during the game. */
int get_nb_scorrs_found();

/** @brief Return the number of squares reachable in all the level discovered. */
int get_nb_squares_reachable();

/** @brief Return the number of square explored during the game. */
int get_nb_squares_explored();

int get_used_moves();

/** @brief Return the maximal number of moves allowed in this game. */
int get_max_moves();

/** @brief The game id is set once the game has been added to the database */
int get_game_id();
...
/** @brief Return the number of levels visited. */
int get_level_reached();

/** @brief Return the name of the bot playing this game. */
const char * get_bot_name();

/** @brief Return the name of the mode used in this game. */
const char * get_mode_name();

/** @brief Return time (in ms) since beginning of the game. */
int get_processing_time();

/** @brief Return the game seed. */
int get_seed();

/** @brief Add result of current game to database */
void gs_submit_game();
